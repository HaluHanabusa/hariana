...
### 5. 結果を眺める
Obsidianの起動画面で「保管庫としてフォルダを開く」→ `output/` を選ぶと、
集めた事実(Claim)とアイデアの系譜がリンクの網として見えます。
詳しい開き方とグラフの見方は、下の「Obsidianでの見方」を参照。
...
### 開き方
1. https://obsidian.md から無料版をインストールする
2. 起動画面の「**保管庫としてフォルダを開く**」→「開く」で、リポジトリ全体では
   なく `output/` フォルダを選ぶ(表記はObsidian 1.12時点。英語版は
   Open folder as vault)。設定フォルダ `.obsidian/` が output/ 内に作られるが、
   gitignore済みなのでリポジトリは汚れない
3. 制限モード(プラグイン無効)の確認が出たらそのままでOK。標準機能だけで足りる

### 見方のコツ
- **グラフビュー(Ctrl+G)**: 事実(Claim)と候補のつながりが網として見える。
  グラフ設定(歯車)→「グループ」に `path:claims` `path:layers`
  `path:synthesis` `path:proposals` を登録して色分けすると、
  「事実の層から候補が生える」構造が色で分かる
- **候補から根拠へ遡る**: synthesis/whitespace-candidates.md を開いて
  右サイドバーの**バックリンク**を見るか、コマンドパレット(Ctrl+P)→
  「ローカルグラフ」。その候補がどのClaimに支えられているかをたどれる
- **frontmatter**(出典・falsifiable_by 等)はファイル上部の「プロパティ」欄に
  表示される。全文検索は Ctrl+Shift+F
- エージェント稼働中に開きっぱなしでよい(外部の変更は自動で反映される)。
